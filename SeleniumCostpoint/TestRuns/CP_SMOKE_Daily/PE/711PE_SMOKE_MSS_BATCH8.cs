...
{
    public class CP711PE_SMOKE_MSS_BATCH8 : SeleniumTestRun
    {
        public CP711PE_SMOKE_MSS_BATCH8(Logger SessionLogger) : base(SessionLogger) { }

        public override void ExecuteTests()
        {
				SetDefaultArgs(SeleniumDriver.Browser.CHROME, "C71MQCM16AE");
				AddTestScripts(new LDMELV_SMOKE());
				AddTestScripts(new LDMEPD_00_PREREQ());
				AddTestScripts(new LDMEPD_SMOKE());
				AddTestScripts(new LDMEPD_04_POSTREQ());
				...
				AddTestScripts(new LDMLAD_00_SMOKE_PREREQ());
				AddTestScripts(new LDMLAD_SMOKE());
				AddTestScripts(new LDMLAD_04_SMOKE_POSTREQ());
