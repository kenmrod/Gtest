...
{
    public class EPMEXPTYPE_SMOKE : SeleniumTestScript
    {
...
new SeleniumControl(sDriver,"Manage Expense Types", "xpath","//div[@class='navItem'][.='Manage Expense Types']").Click();
				...
				sDriver.SessionLogger.WriteLine("[EPMEXPTYPE] Perfoming VerifyExists on MainForm...", Logger.MessageType.INF);
				SeleniumControl EPMEXPTYPE_MainForm = new SeleniumControl( sDriver, "MainForm", "xpath", "//div[@id='0']/form[1]");
				Function.AssertEqual(true,EPMEXPTYPE_MainForm.Exists());
				...
				sDriver.SessionLogger.WriteLine("[EPMEXPTYPE] Perfoming VerifyExists on ExpenseTypeCode...", Logger.MessageType.INF);
				...
				Function.AssertEqual(true,EPMEXPTYPE_ExpenseTypeCode.Exists());
				...
				sDriver.SessionLogger.WriteLine("[EPMEXPTYPE] Perfoming Close on MainForm...", Logger.MessageType.INF);
				SeleniumControl EPMEXPTYPE_MainForm = new SeleniumControl( sDriver, "MainForm", "xpath", "//div[@id='0']/form[1]");
				Function.WaitControlDisplayed(EPMEXPTYPE_MainForm);
IWebElement formBttn = EPMEXPTYPE_MainForm.mElement.FindElements(By.CssSelector("*[title*='Close']")).Where(x=>x.Displayed).FirstOrDefault();
