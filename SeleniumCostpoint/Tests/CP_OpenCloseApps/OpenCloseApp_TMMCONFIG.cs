...
{
    public class OpenCloseApp_TMMCONFIG : SeleniumTestScript
    {
        public override bool TestExecute(out string ErrorMessage, string TestEnvironment, IDriver Driver)
        {
			...
			try
			{
				...
				this.ScriptLogger.WriteLine("[TMMCONFIG] Opening app...", Logger.MessageType.INF);
				Thread.Sleep(3000);
				SearchApplications.SendKeys("TMMCONFIG",true);
				Thread.Sleep(500);
				SearchAppResultList.FindElement();
				SearchAppResultList.mElement.FindElements(By.XPath(".//div[@class='autoCItem'][contains(@style,'block')][contains(.,'(TMMCONFIG)')]")).FirstOrDefault().Click();
				Function.WaitControlDisplayed(MainForm,10);
				this.ScriptLogger.WriteLine("[TMMCONFIG] Performing Close on MainForm...", Logger.MessageType.INF);
