...
{
    public class OpenCloseApp_SMMSETA : SeleniumTestScript
    {
        public override bool TestExecute(out string ErrorMessage, string TestEnvironment, IDriver Driver)
        {
			...
			try
			{
				...
				this.ScriptLogger.WriteLine("[SMMSETA] Opening app...", Logger.MessageType.INF);
				Thread.Sleep(3000);
				SearchApplications.SendKeys("SMMSETA",true);
				Thread.Sleep(500);
				SearchAppResultList.FindElement();
				SearchAppResultList.mElement.FindElements(By.XPath(".//div[@class='autoCItem'][contains(@style,'block')][contains(.,'(SMMSETA)')]")).FirstOrDefault().Click();
				Function.WaitControlDisplayed(MainForm,10);
				this.ScriptLogger.WriteLine("[SMMSETA] Performing Close on MainForm...", Logger.MessageType.INF);
