...
{
    public class OpenCloseApp_PJPUPLAB : SeleniumTestScript
    {
        public override bool TestExecute(out string ErrorMessage, string TestEnvironment, IDriver Driver)
        {
			...
			try
			{
				...
				this.ScriptLogger.WriteLine("[PJPUPLAB] Opening app...", Logger.MessageType.INF);
				Thread.Sleep(3000);
				SearchApplications.SendKeys("PJPUPLAB",true);
				Thread.Sleep(500);
				SearchAppResultList.FindElement();
				SearchAppResultList.mElement.FindElements(By.XPath(".//div[@class='autoCItem'][contains(@style,'block')][contains(.,'(PJPUPLAB)')]")).FirstOrDefault().Click();
