...
{
    public class OpenCloseApp_SYMCFLA : SeleniumTestScript
    {
        public override bool TestExecute(out string ErrorMessage, string TestEnvironment, IDriver Driver)
        {
			...
			SeleniumControl MainForm = new SeleniumControl( sDriver, "MainForm", "xpath", "//div[@id='2']/form[1]");
			...
			try
			{
				...
				this.ScriptLogger.WriteLine("[SYMCFLA] Opening app...", Logger.MessageType.INF);
				Thread.Sleep(3000);
				SearchApplications.SendKeys("SYMCFLA",true);
				Thread.Sleep(500);
				SearchAppResultList.FindElement();
				SearchAppResultList.mElement.FindElements(By.XPath(".//div[@class='autoCItem'][contains(@style,'block')][contains(.,'(SYMCFLA)')]")).FirstOrDefault().Click();
				Function.WaitControlDisplayed(MainForm,10);
				this.ScriptLogger.WriteLine("[SYMCFLA] Performing Close on MainForm...", Logger.MessageType.INF);
