...
{
    public class OpenCloseApp_LDPBLPTS : SeleniumTestScript
    {
        public override bool TestExecute(out string ErrorMessage, string TestEnvironment, IDriver Driver)
        {
			...
			try
			{
				...
				this.ScriptLogger.WriteLine("[LDPBLPTS] Opening app...", Logger.MessageType.INF);
				Thread.Sleep(3000);
				SearchApplications.SendKeys("LDPBLPTS",true);
				Thread.Sleep(500);
				SearchAppResultList.FindElement();
				SearchAppResultList.mElement.FindElements(By.XPath(".//div[@class='autoCItem'][contains(@style,'block')][contains(.,'(LDPBLPTS)')]")).FirstOrDefault().Click();
				Function.WaitControlDisplayed(MainForm,10);
				this.ScriptLogger.WriteLine("[LDPBLPTS] Performing Close on MainForm...", Logger.MessageType.INF);
