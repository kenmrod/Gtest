...
{
    public class OpenCloseApp_EPQCHGEXPSTAT : SeleniumTestScript
    {
        public override bool TestExecute(out string ErrorMessage, string TestEnvironment, IDriver Driver)
        {
			...
			SeleniumControl MainForm = new SeleniumControl( sDriver, "MainForm", "xpath", "//div[@id='1']/form[1]");
			...
			try
			{
				...
				this.ScriptLogger.WriteLine("[EPQCHGEXPSTAT] Opening app...", Logger.MessageType.INF);
				Thread.Sleep(3000);
				SearchApplications.SendKeys("EPQCHGEXPSTAT",true);
				Thread.Sleep(500);
				SearchAppResultList.FindElement();
				SearchAppResultList.mElement.FindElements(By.XPath(".//div[@class='autoCItem'][contains(@style,'block')][contains(.,'(EPQCHGEXPSTAT)')]")).FirstOrDefault().Click();
				Function.WaitControlDisplayed(MainForm,10);
				this.ScriptLogger.WriteLine("[EPQCHGEXPSTAT] Performing Close on MainForm...", Logger.MessageType.INF);
