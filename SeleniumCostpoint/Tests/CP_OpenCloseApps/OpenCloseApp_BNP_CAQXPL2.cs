...
{
    public class OpenCloseApp_BNP_CAQXPL2 : SeleniumTestScript
    {
        public override bool TestExecute(out string ErrorMessage, string TestEnvironment, IDriver Driver)
        {
			...
			try
			{
				...
				this.ScriptLogger.WriteLine("[BNP_CAQXPL2] Opening app...", Logger.MessageType.INF);
				Thread.Sleep(3000);
				SearchApplications.SendKeys("BNP_CAQXPL2",true);
				Thread.Sleep(500);
				SearchAppResultList.FindElement();
				SearchAppResultList.mElement.FindElements(By.XPath(".//div[@class='autoCItem'][contains(@style,'block')][contains(.,'(BNP_CAQXPL2)')]")).FirstOrDefault().Click();
				Function.WaitControlDisplayed(MainForm,10);
				this.ScriptLogger.WriteLine("[BNP_CAQXPL2] Performing Close on MainForm...", Logger.MessageType.INF);
